...
namespace SV22T1020607.BusinessLayers
{
    ...
    {
        private static IGenericRepository<Supplier> supplierDB = null!;
        private static ICustomerRepository customerDB = null!;
        private static IGenericRepository<Shipper> shipperDB = null!;
    ...
    /// </param>
    /// <returns></returns>
    public static Task<bool> ValidatelCustomerEmailAsync(string email, int customerID = 0)
    {
        return customerDB.ValidateEmailAsync(email, customerID);
    }
    
    // BACKWARD COMPATIBILITY SYNC METHODS
    public static Customer? GetCustomer(int id) => GetCustomerAsync(id).Result;
    public static bool DeleteCustomer(int id) => DeleteCustomerAsync(id).Result;
    public static Supplier? GetSupplier(int id) => GetSupplierAsync(id).Result;
    public static int AddSupplier(Supplier data) => AddSupplierAsync(data).Result;
    public static bool UpdateSupplier(Supplier data) => UpdateSupplierAsync(data).Result;
    public static bool DeleteSupplier(int id) => DeleteSupplierAsync(id).Result;
    
    public static List<Supplier> ListOfSuppliers(int page = 1, int pageSize = 0, string searchValue = "")
    {
        return ListSuppliersAsync(new PaginationSearchInput { Page = page, PageSize = pageSize, SearchValue = searchValue }).Result.DataItems.ToList();
    }
    
    public static int CountSuppliers(string searchValue = "")
    {
        return ListSuppliersAsync(new PaginationSearchInput { Page = 1, PageSize = 1, SearchValue = searchValue }).Result.RowCount;
    }
